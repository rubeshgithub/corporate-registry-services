---
title: "Free NUANS Name Search Canada | Instant Availability Check"
metaDescription: "Free instant availability check across Canadian corporate registries. Order the official {{price:nuans-search}} NUANS report from the same page. No email required for the check."
slug: "/incorporation/nuans-name-search-canada"
primaryKeyword: "NUANS name search Canada"
secondaryKeywords:
  - "free NUANS search"
  - "NUANS name canada"
  - "NUANS report Canada"
  - "NUANS name reservation"
  - "how to read a NUANS report"
  - "how long is a NUANS report good for"
  - "what is NUANS"
  - "NUANS meaning"
  - "NUANS pre-check"
  - "Canadian corporate name check"
  - "canada corporate name search"
  - "NUANS name search bc"
  - "ontario NUANS name search"
  - "NUANS search alberta"
  - "nuans corporate name search"
  - "free corporate name search Canada"
jurisdiction: "Canada"
cluster: "incorporation"
lastUpdated: "2026-07-24"
schemaSuggestions: ["Service", "FAQPage", "BreadcrumbList"]
---

# Free NUANS Name Search: Instant Availability Check for Canadian Corporations

Check how many Canadian corporations already use a similar name — free, instant, across federal + BC + Alberta live registries. A courtesy pre-check to help you decide whether to invest in the official NUANS report before ordering it.

## What is a NUANS report?

**NUANS** stands for *Newly Updated Automated Name Search* — the federal name-search system operated by Innovation, Science and Economic Development Canada (ISED). It's the authoritative Canadian corporate name search, required by Corporations Canada for federal (CBCA) incorporation, cross-provincial extra-provincial registration, and most corporate name changes. Provincial registrars in Alberta, Nova Scotia, Manitoba, New Brunswick, Newfoundland, and Yukon also require or accept a NUANS.

## What the free check gets you

A traffic-light indicator (strong / moderate / weak), the list of similar corporations we found, and a direct path to order the official NUANS report ({{price:nuans-search}} all-in + GST, delivered within one business hour) — which registrars require to accept your incorporation filing. Valid for 90 days from issue.

## Related services

- [Federal vs. Provincial Incorporation in Canada](/articles/federal-vs-provincial-incorporation-canada)
- [Order a NUANS Name Search Report](/order/nuans-search)
- [Federal (CBCA) Incorporation Service](/incorporation/canada-federal-incorporation-service)
- [Book a Free Incorporation Consultation](/incorporation/book-free-consultation)
