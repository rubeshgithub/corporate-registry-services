 ...
 * Handles the 4 form-based document services:
 *   - share-certificate      $49
 *   - director-resolution    $79
 *   - shareholder-resolution $79
 *   - bylaws                 $99
 *
 * Same shape as /api/order/change-request. Details payload is
 * service-specific and passed through to Stripe metadata (chunked to fit
 * the 500-char metadata-value limit) so the fulfillment team can prepare
 * the document from the checkout notification without a separate query.
 */
...
if (USE_TEST_PRICE) {
  console.warn(`[order/corp-doc] TEST PRICE ACTIVE: charging ${TEST_OVERRIDE_CENTS} cents for every corp-doc order.`);
}

function isValid(body: Body): string | null {
  if (!body?.service || !(body.service in CORP_DOC_CONFIGS)) return "Invalid or missing service.";
...
export async function POST(req: Request) {
  ...
  try {
    ...
    console.error("[order/corp-doc] Stripe error:", msg);
