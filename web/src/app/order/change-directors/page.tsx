...
export async function generateMetadata(): Promise<Metadata> {
  const cents = await getPriceCents("change-directors");
  return {
    ...BASE_METADATA,
...
export default async function ChangeDirectorsOrderPage() {
  return (
    <>
      <Header />
      <main style={{ flex: 1, background: "var(--bg)" }}>
        <Suspense fallback={<div style={{ maxWidth: 620, margin: "0 auto", padding: "3rem 1.5rem", textAlign: "center", color: "var(--text-muted)" }}>Loading…</div>}>
          <ChangeOrderFlow config={await withLivePrice(CHANGE_CONFIGS["change-directors"], "change-directors")} />
        </Suspense>
      </main>
      <Footer />
    </>
