...
const BASE_METADATA: Metadata = {
  ...
  description:
    "Revive a dissolved or struck-off Canadian corporation with the government registry. $399 all-in + GST. Application filed within 1 business day.",
...
export async function generateMetadata(): Promise<Metadata> {
  const cents = await getPriceCents("revival");
  return {
    ...BASE_METADATA,
...
export default async function RevivalOrderPage() {
  return (
    <>
      <Header />
      <main style={{ flex: 1, background: "var(--bg)" }}>
        <Suspense fallback={<div style={{ maxWidth: 620, margin: "0 auto", padding: "3rem 1.5rem", textAlign: "center", color: "var(--text-muted)" }}>Loading…</div>}>
          <ChangeOrderFlow config={await withLivePrice(CHANGE_CONFIGS["revival"], "revival")} />
        </Suspense>
      </main>
      <Footer />
    </>
