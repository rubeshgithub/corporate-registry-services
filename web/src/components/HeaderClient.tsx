 ...
 * ships, drop it into buildNav as a new dropdown with the section's real
 * routes; the desktop and mobile renderers pick it up automatically.
 *
 * Prices in the hints come from the pricing catalogue via the server shell
 * in Header.tsx. They used to be string literals here, which is how the nav
 * on every page kept quoting $99 / $49 / $79 for weeks after the catalogue
 * moved to $129 / $69 / $109.
 */
...
const dollars = (cents: number) => `$${Math.round(cents / 100).toLocaleString("en-CA")}`;

/* A hint never shows a broken "· all-in" if a key is somehow missing —
   it just drops the price segment. */
const priced     = (lead: string, cents?: number, tail = "") => cents == null ? lead : `${lead} · ${dollars(cents)}${tail}`;
const fromPriced = (lead: string, cents?: number)            => cents == null ? lead : `${lead} · from ${dollars(cents)}`;

function buildNav(p: Prices): NavItem[] {
  return [
    {
      kind:  "dropdown",
      label: "Services",
      items: [
        ...
        { label: "Professional Corporations",   href: "/professional-corporation",  hint: fromPriced("Doctors, dentists, lawyers, CPAs", p["pc-profile-report"]) },
        { label: "Certificates of Good Standing", href: "/good-standing",           hint: priced("Government-issued", p["good-standing"], " all-in") },
        { label: "Director / Officer Changes",  href: "/order/change-directors",    hint: "Update the registry after board changes" },
        { label: "Registered Address Changes",  href: "/order/change-address",      hint: "Move your registered office, filed same business day" },
    ...
    },
    {
      ...
      items: [
        ...
        { label: "NFP Grants",   href: "/nfp-grants",   hint: "Government grants for not-for-profits" },
...
export default function HeaderClient({ prices }: { prices: Prices }) {
  const nav = buildNav(prices);
  ...
  return (
    ...
    >
      ...
      {mobileOpen && <MobileDrawer nav={nav} onClose={() => setMobileOpen(false)} />}
    </header>
  );
}
...
function MobileDrawer({ nav, onClose }: { nav: NavItem[]; onClose: () => void }) {
