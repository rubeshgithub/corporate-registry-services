 ...
 * policy in /disclaimer, a jurisdiction rule in an article, etc.
 *
 * Prices are NOT written here. Any answer that quotes one takes it from the
 * `price(key)` callback, which the /faq page builds from the live catalogue.
 * The old version had "$99" / "$49" / "$79" as literals, and after the Sep
 * 2026 price change the FAQ — and the FAQPage schema Google reads from it —
 * kept quoting numbers the checkout no longer charged.
 *
...
/** Formats a catalogue key as a display price, e.g. "$129". */
export type PriceOf = (key: string) => string;
  ...
  return [
    {
      ...
      items: [
        ...
        },
        {
          ...
          a: `Numbered incorporation is ${price("incorporation-numbered")} all-in + GST. Named incorporation is ${price("incorporation-named")} all-in + GST (includes a NUANS name search). Extra-provincial registration is ${price("extra-provincial")} all-in + GST. Government filing fees are included.`,
          href: "/order/incorporation",
        },
        {
          q: "How much is a corporate profile report?",
          a: `${profile} all-in + GST. Delivered as a PDF, direct from the government registry, within one business hour.`,
          href: "/order/profile-report",
        },
        {
          q: "How much is a Certificate of Good Standing?",
          a: `${goodSt} all-in + GST. Government-issued, delivered as a PDF within hours.`,
          href: "/order/good-standing",
        },
        {
          q: "How much do dissolution and revival services cost?",
          a: `Voluntary dissolution is ${price("voluntary-dissolution")} all-in + GST and corporate revival is ${price("revival")} all-in + GST. Revival often involves catching up on missed annual returns — those are quoted separately at the standard ${annual} per year.`,
    ...
    },
    {
      ...
      items: [
        ...
        },
        {
          ...
          a: `A profile report is a detailed snapshot of all the public information about the corporation (multi-page, ~${profile}). A Certificate of Good Standing is a single-page official confirmation that the corporation is active and compliant as of the issue date (~${goodSt}). Banks and law firms usually ask for the certificate; due diligence usually needs the full report.`,
          href: "/guides/corporate-profile-report-vs-certificate-of-good-standing",
        },
        {
